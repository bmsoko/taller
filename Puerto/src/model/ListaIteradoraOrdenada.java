...
public class ListaIteradoraOrdenada extends ListaDobleIter {
	...
	public ListaIteradoraOrdenada(ListaEnlazadaDoble lista)
			throws ClassCastException {
		super(lista);
		super.actual = super.lista.esVacia() ? super.lista.getCabecera()
				: super.lista.getCabecera().getSiguiente();
		...
		if (!super.lista.esVacia()) {
			while (super.actual.getSiguiente() != null) {
				if (super.actual.getDato().compareTo(dato) >= 0) {
					...
					NodoListaDoble tmp = new NodoListaDoble(dato,
							super.getActual(), super.actual.getAnterior());
			...
			// Lista vacia
			NodoListaDoble tmp = new NodoListaDoble(dato, this.lista.getFin(),
					this.lista.getCabecera());
